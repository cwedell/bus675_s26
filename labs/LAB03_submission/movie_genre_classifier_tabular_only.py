...
class MultimodalGenreClassifier(nn.Module):
    ...
    def __init__(self, vocab_sizes):
        ...
        self.fusion_head = FusionHead(image_dim=0, tabular_dim=256, num_classes=len(GENRES))

    def forward(self, image, numeric, cat_fields):
        # YOUR CODE HERE
        # image_features = self.image_branch(image)
        tabular_features = self.tabular_branch(numeric, cat_fields)
        dummy = torch.zeros(numeric.size(0), 0, device=numeric.device)
        return self.fusion_head(dummy, tabular_features)
