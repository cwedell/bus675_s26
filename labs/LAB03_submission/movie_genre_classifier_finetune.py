...
class MultimodalGenreClassifier(nn.Module):
    ...
    def __init__(self, vocab_sizes):
        super().__init__()
        # YOUR CODE HERE
        self.image_branch = ImageBranch(out_dim=256, fine_tune=True)
        self.tabular_branch = TabularBranch(vocab_sizes=vocab_sizes, out_dim=256)
        self.fusion_head = FusionHead(image_dim=256, tabular_dim=256, num_classes=len(GENRES))

    def forward(self, image, numeric, cat_fields):
        # YOUR CODE HERE
        image_features = self.image_branch(image)
        tabular_features = self.tabular_branch(numeric, cat_fields)
        return self.fusion_head(image_features, tabular_features)
...
for epoch in range(PHASE1_EPOCHS):
    ...
    with torch.no_grad():
        ...
        print(f"Epoch {epoch+1}/{PHASE1_EPOCHS}, Train Loss: {avg_loss:.4f}, Train Accuracy: {(100 * train_acc):.2f}%, Val Accuracy: {(100 * val_acc):.2f}%")
...
# Unfreeze layer 4
for param in model.image_branch.backbone[-2].parameters():
    param.requires_grad = True

backbone_params = [p for p in model.image_branch.backbone.parameters() if p.requires_grad]
head_params     = list(model.image_branch.head.parameters())
other_params    = list(model.tabular_branch.parameters()) + list(model.fusion_head.parameters())
...
PHASE2_EPOCHS = 5
print("Starting fine-tuning phase with unfrozen backbone...")
...
for epoch in range(PHASE2_EPOCHS):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            correct += (predicted == labels).sum().item()
            val_acc = correct / total
        print(f"Epoch {epoch+1}/{PHASE2_EPOCHS}, Train Loss: {avg_loss:.4f}, Train Accuracy: {(100 * train_acc):.2f}%, Val Accuracy: {(100 * val_acc):.2f}%")
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), "best_model.pth")
            print("Saved new best checkpoint!")
